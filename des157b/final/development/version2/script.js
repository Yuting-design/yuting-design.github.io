(function () {
    ...
    document.querySelector('#dateDetialDisplayer').innerHTML = todayIs + `<button class="closeBtn" type='button'>Close</button>`;
    //get mood
    const mood = ['good', 'bad', 'dontKnow']
    const color = ['deeppink', 'mediumblue', 'darkviolet']
    //add new 
    //form pages
    ...
    async function showDaydetial(recordId) {
        ...
        try {
            ...
            theDetial.innerHTML = `
                <div class="feeling">Feel ${feeling}</div>
                <div class="level">Level ${level}</div>                
                <div class="detials">${detials}</div>
                <button class="closeBtn" type='button'>Close</button>`;
            popWindow.append(theDetial);
            for (let i = 0; i < mood.length; i++) {
                if (mood[i] == feeling) {
                    document.querySelector('.theDetial').style.backgroundColor = color[i];
                }
            }
        } catch (error) {
    ...
    async function displaySpDays() {
        ...
        try {
            ...
            results.forEach(function (eachSpDay) {
                ...
                <div class="todayIs">${spday}</div>
                <button class="deleteBtn" type='button'>Delete</button>`;

                spdayDisplayer.append(theListItem);
                for (let i = 0; i < mood.length; i++) {
                    if (mood[i] == feeling) {
                        document.getElementById(`r-${id}`).style.backgroundColor = color[i];
                    }
                }
            });
        } catch {
